...
import './dashboards/BusinessDashboard.ref.css';
...
🎉 Agent/Manager added successfully!
...
🔐 Staff can now login at: /staff/login
  ...
  const handleStartEditAgent = async (agent) => {
    ...
    if (agent.role === 'support_manager') {
      try {
        const res = await fetch(`${API_BASE}/departments/manager-permissions/${agent.id}`);
                  ...
                  products.map(product => (
                    <tr key={product.id} style={{ borderBottom: '1px solid #e9ecef' }}>
                      ...
                      <td style={{ 
                        ...
                        {formatDateIST(product.created_at)}
                      </td>
                      <td style={{ 
                        width: '20%', 
                        padding: '15px', 
                      ...
                      {modules.map(module => (
                        <tr key={module.id}>
                          ...
                          <td className="created-date">
                            {formatDateIST(module.created_at)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="add-agent-form">
              ...
              <form onSubmit={handleAddAgent}>
                ...
                <div className="form-group">
                  ...
                  <select
                    value={newAgent.primary_department_id}
                    onChange={(e) => setNewAgent({...newAgent, primary_department_id: e.target.value})}
                    className="form-select"
                    required
                  >
                ...
                {newAgent.role === 'support_agent' && (
                  <>
                    <div className="form-group">
                      ...
                      <select
                        value={newAgent.level || 'L1'}
                        onChange={(e) => setNewAgent({ ...newAgent, level: e.target.value })}
                        className="form-select"
                      >
                    ...
                    <div className="form-group">
                      ...
                      >
                        ...
                        {agents.filter(a => a.role === 'support_manager').map(mgr => (
                ...
                {newAgent.role === 'support_manager' && newAgent.primary_department_id && (
                  <div className="permissions-grid-container" style={{ margin: '15px 0', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      {departments.filter(d => d.id !== Number(newAgent.primary_department_id)).map(dept => {
                        const perm = newAgent.additional_permissions.find(p => p.department_id === dept.id) || {
                        ...
                        const togglePermission = (field) => {
                          const existingIndex = newAgent.additional_permissions.findIndex(p => p.department_id === dept.id);
                          let updatedPermissions = [...newAgent.additional_permissions];
            ...
            <div className="edit-agent-form">
              ...
              <form onSubmit={handleEditAgent}>
                ...
                <div className="form-group">
                  <label>Primary Department *</label>
                  <select
                    value={editingAgent.primary_department_id}
                    onChange={(e) => setEditingAgent({...editingAgent, primary_department_id: e.target.value})}
                    className="form-select"
                    required
                  >
                    <option value="">Select Department</option>
                ...
                {editingAgent.role === 'support_agent' && (
                  <>
                    <div className="form-group">
                      <label>Support level</label>
                      <select
                        value={editingAgent.level || 'L1'}
                        onChange={(e) => setEditingAgent({ ...editingAgent, level: e.target.value })}
                        className="form-select"
                      >
                        <option value="L1">L1 — First line</option>
                        <option value="L2">L2 — Second line</option>
                    ...
                    <div className="form-group">
                      <label>Reporting Manager</label>
                      <select
                        value={editingAgent.manager_id}
                        onChange={(e) => setEditingAgent({...editingAgent, manager_id: e.target.value})}
                        className="form-select"
                      >
                        <option value="">Select Manager (None)</option>
                ...
                {editingAgent.role === 'support_manager' && editingAgent.primary_department_id && (
                  <div className="permissions-grid-container" style={{ margin: '15px 0', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <h4 style={{ margin: '0 0 5px 0' }}>Additional Department Permissions</h4>
                    <p style={{ fontSize: '0.85em', color: '#64748b', margin: '0 0 15px 0' }}>Configure override permission matrices for other departments.</p>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      {departments.filter(d => d.id !== Number(editingAgent.primary_department_id)).map(dept => {
                        const perm = editingAgent.additional_permissions.find(p => p.department_id === dept.id) || {
                          department_id: dept.id,
                          can_view: 0,
                          can_update: 0,
                          can_assign: 0,
                        ...
                        const togglePermission = (field) => {
                          const existingIndex = editingAgent.additional_permissions.findIndex(p => p.department_id === dept.id);
                          let updatedPermissions = [...editingAgent.additional_permissions];
              ...
              <tbody>
                {agents.map(agent => (
                  <tr key={agent.id}>
                    ...
                    <td>{agent.role === 'support_agent' ? (agent.level || 'L1') : '—'}</td>
                    <td>
                    ...
                    <td>
                      {agent.last_login ? 
                        formatDateIST(agent.last_login)
                        : 'Never'
                      }
                    </td>
