...
const CEODashboard = ({ ceo, onLogout }) => {
  ...
  const fetchExecutiveData = async (isRefresh = false) => {
    try {
      ...
      const [ticketsResponse, departmentsResponse, agentsResponse] = await Promise.all([
        ...
        fetch(buildApiUrl('/api/agents?role=support_manager'), { headers }),
        fetch(buildApiUrl('/api/agents?role=support_agent'), { headers })
      ]);
      ...
      if (agentsResponse.ok) {
        ...
        agentsList = agentsData.success
          ? (agentsData.data || []).filter((agent) => agent.is_active)
          : [];
  ...
  // Fetch data when switching to products or organizations tab
  useEffect(() => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('/api/sla/products'), {
        ...
        body: JSON.stringify({
          ...
          organization_ids: newProduct.organization_ids
        })
      });
      const data = await response.json();
      if (data.success) {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`/api/sla/products/${editingProduct.id}`), {
        ...
        body: JSON.stringify({
          ...
          organization_ids: editingProduct.organization_ids
        })
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleAddOrg = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setShowAddOrg(false);
        fetchOrganizations();
        showSuccessNotification('Organization added successfully!');
      } else {
        showErrorNotification(data.message || 'Failed to add organization');
      }
    } catch (error) {
      console.error('Error adding organization:', error);
      showErrorNotification('Failed to add organization');
    } finally {
  ...
  const handleEditOrg = async (e) => {
    ...
    try {
      const headers = getAuthHeaders();
      const response = await fetch(buildApiUrl(`/api/organizations/${editingOrg.id}`), {
        method: 'PUT',
        headers: { ...headers, 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        showErrorNotification(data.message || 'Failed to update organization');
      }
    } catch (error) {
  ...
  const handleDeleteOrg = async (orgId) => {
    const confirmed = await confirmAction('Confirm Delete', 'Are you sure you want to delete this organization? This action cannot be undone.');
    if (!confirmed) return;
    try {
      const headers = getAuthHeaders();
      ...
      if (data.success) {
        fetchOrganizations();
        showSuccessNotification('Organization deleted successfully!');
      } else {
        showErrorNotification(data.message || 'Failed to delete organization');
      }
    } catch (error) {
      console.error('Error deleting organization:', error);
      showErrorNotification('Failed to delete organization');
    }
  };
  ...
  const fetchAgents = async () => {
    try {
      const headers = getAuthHeaders();
      const response = await fetch(buildApiUrl('/api/agents?role=support_agent'), { headers });
      const data = await response.json();
      if (data.success) {
  ...
  const handleAddAgent = async (e) => {
    ...
    try {
      ...
      const agentData = {
        ...
        level: newAgent.role === 'support_manager' ? null : (['L1', 'L2', 'L3'].includes(lv) ? lv : 'L1'),
      ...
      if (data.success) {
        ...
        const credentialsMessage = `Agent/Manager added successfully! Login ID: ${credentials.login_id}, Password: ${credentials.password}. Please save these credentials securely.`;
        showSuccessNotification(credentialsMessage);
      } else {
  ...
  const handleEditAgent = async (e) => {
    ...
    try {
      ...
      const agentData = {
        ...
        level: editingAgent.role === 'support_manager' ? null : (['L1', 'L2', 'L3'].includes(lv) ? lv : 'L1'),
  ...
  const handleStartEditAgent = async (agent) => {
    ...
    if (agent.role === 'support_manager') {
      try {
        const headers = getAuthHeaders();
        const res = await fetch(buildApiUrl(`/api/departments/manager-permissions/${agent.id}`), { headers });
    ...
    console.log('Editing agent skills:', agent.skills);
    setEditingAgent({
      ...agent,
  ...
  const tabList = [
    ...
    { key: 'organizations', label: 'Organizations', icon: 'departments', trackUnread: false },
              ...
              <div className="adr-dashboard">
                <div className="adr-dashboard__head">
                  ...
                  <p className="adr-dashboard__sub">
                    Organization-wide ticket metrics
                    {lastUpdated ? (
                    ...
                    <div className="ceo-form-group">
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {organizations.map((org) => (
                          <label key={org.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                            <input
                              type="checkbox"
                              checked={(editingProduct ? editingProduct.organization_ids : newProduct.organization_ids).includes(org.id)}
                              onChange={(e) => {
                                const orgIds = editingProduct ? editingProduct.organization_ids : newProduct.organization_ids;
                                const updatedOrgIds = e.target.checked
                                  ? [...orgIds, org.id]
                                  : orgIds.filter(id => id !== org.id);
                                if (editingProduct) {
                                  setEditingProduct({ ...editingProduct, organization_ids: updatedOrgIds });
                                } else {
                                  setNewProduct({ ...newProduct, organization_ids: updatedOrgIds });
                                }
                              }}
                            />
                            {org.name}
                      ...
                      <div className="ceo-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="ceo-form-group">
                          ...
                          >
                            ...
                            {modules.filter(m => m.product_id === (editingSLA ? editingSLA.product_id : newSLA.product_id)).map((module) => (
