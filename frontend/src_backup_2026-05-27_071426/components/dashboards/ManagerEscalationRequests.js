...
const ManagerEscalationRequests = () => {
  ...
  useEffect(() => {
    if (modalConfig.isOpen && modalConfig.action === 'approve' && modalConfig.targetLevel) {
  ...
  const handleSubmitAction = async () => {
    const { requestId, action, comment, assignmentMode, selectedAgentId } = modalConfig;
    if (!requestId || !action) return;

    if (action === 'approve' && assignmentMode === 'manual' && !selectedAgentId) {
    ...
    try {
      ...
      if (action === 'approve') {
        ...
        body.target_level = modalConfig.targetLevel;
      }
