...
const ManagerDashboard = ({ manager, onLogout }) => {
    ...
    try {
      ...
      if (ticketsResponse.ok) {
        ...
        if (ticketsData.length > 0) {
          try {
            const progressRes = await fetch('http://localhost:5000/api/ticket-tasks/progress/bulk', {
              method: 'POST',
              headers: headers,
  ...
  const fetchPendingMailCount = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/mail-review', {
        method: 'GET',
  ...
  const fetchTicketReplies = async (ticketId) => {
    try {
      ...
      const chatResponse = await fetch(`http://localhost:5000/api/chat/messages/${ticketId}`, {
        method: 'GET',
        headers: headers
      });
