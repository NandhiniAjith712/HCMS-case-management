...
const TicketDetailPage = ({ user: propUser, accessScope = null }) => {
  ...
  const handleSubmitResolutionAndResolve = async () => {
    if (childWorkflowLocked) {
      showInlineNotification(
        `This ticket is linked to parent #${ticket?.parent_ticket_id}. Resolve from the parent ticket.`,
        'error',
        3500
      );
      return;
    ...
    if (linkedWorkflowLocked) {
      showInlineNotification(
        `This ticket is linked under primary ticket #${linkedPrimaryTicketId}. Resolve from the primary ticket.`,
        'error',
        3500
      );
      return;
    ...
    try {
      const response = await fetch(buildApiUrl(`/api/tickets/${ticketId}/status`), {
        ...
        body: JSON.stringify({
          status: 'resolved',
          resolution_details: getResolutionPayload()
        })
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data.success) {
        showInlineNotification(data.message || 'Failed to resolve ticket.', 'error', 4000);
        return;
      }
      setShowResolutionModal(false);
      await fetchTicketDetails();
      await fetchActivity();
      showStatusChangeNotification('resolved');
    } catch (error) {
      console.error('Error resolving ticket:', error);
      showInlineNotification('Error resolving ticket. Please try again.', 'error', 4000);
    } finally {
      setResolutionSaving(false);
    }
  };
    ...
    <div className="ticket-detail-modern-page">
      ...
      <div className="ticket-detail-modern-card">
        <div className="ticket-detail-modern-header">
          ...
          {!isCustomer && String(ticket?.reopen_reason || '').trim() ? (
            <section className="td-section" style={{ marginTop: 12 }}>
              <h3 className="td-section-title">Reopen reason</h3>
              <div className="td-section-divider" aria-hidden="true" />
              <div className="td-muted" style={{ whiteSpace: 'pre-wrap' }}>
        ...
        {!isCustomer && isAgent && !isManager && hasGroupedTasks && (
          <section className="td-section td-grouped-agent-task">
            ...
            <p className="td-muted" style={{ marginTop: 8 }}>
              Complete only your assignment below. The manager marks the ticket resolved after all agents finish.
            </p>
          </section>
        )}
            ...
            <div className="td-confirm-actions">
              ...
              {resolutionMode === 'resolve' && (
                <button
                  type="button"
                  className="td-btn td-btn-primary"
                  onClick={handleSubmitResolutionAndResolve}
                  disabled={resolutionSaving}
                >
                  {resolutionSaving ? 'Saving...' : 'Save & Mark Resolved'}
                </button>
              )}
            </div>
          </div>
