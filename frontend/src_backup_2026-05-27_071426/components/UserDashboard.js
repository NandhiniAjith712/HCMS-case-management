...
const UserDashboard = ({ user }) => {
  ...
  const fetchSpocsAndProducts = async () => {
    if (!currentUser || currentUser.role !== 'org_spoc' || !currentUser.organization_id) return;
    setSpocLoading(true);
    setSpocError(null);
    try {
      const headers = getAuthHeaders();
      
      // Fetch products mapped to organization
      const prodRes = await fetch(`http://localhost:5000/api/organizations/${currentUser.organization_id}/products`, {
        headers
      });
      ...
      const spocRes = await fetch(`http://localhost:5000/api/users/spoc/products`, {
        headers
      });
  ...
  const handleCreateProductSpoc = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/users/spoc/product', {
        method: 'POST',
        headers,
        body: JSON.stringify({
          name: spocForm.name,
  ...
  const handleRevokeSpoc = async (spocUserId) => {
    ...
    setSpocSuccess(null);
    try {
      const res = await fetch(`http://localhost:5000/api/users/spoc/${spocUserId}`, {
        method: 'DELETE',
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        try {
          ...
          if (ids.length) {
            const r = await authenticatedFetch('http://localhost:5000/api/sla/timers/bulk-remaining', {
              method: 'POST',
