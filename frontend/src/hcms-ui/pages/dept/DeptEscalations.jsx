...
export default function DeptEscalations() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        const res = await fetch(`/api/v2/cases?${params.toString()}`, { headers });
        if (!res.ok) throw new Error('Failed to fetch escalated tickets');
        const data = await res.json();
        const allCases = data.cases || data.data || [];
        setTickets(allCases);

        const cats = [...new Set(allCases.map(t => t.category).filter(Boolean))];
        setCategories(cats);
      } catch (err) {
    ...
    return (
      <div style={{ fontFamily: f, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: '#EF4444', marginBottom: 8 }}>Error loading escalated tickets</div>
    ...
    <div style={{ fontFamily: f }}>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 2px rgba(15,23,42,0.04)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {tickets.length === 0 ? (
              <tr><td colSpan={9} style={{ padding: 40, textAlign: 'center', color: '#94A3B8', fontSize: 13 }}>No tickets found</td></tr>
            ) : tickets.map((t, i) => (
              <tr key={t.id} onClick={() => navigate(`/hcms/dept-escalations/${t.id}`)}
                ...
                <td style={{ padding: '14px 16px', fontSize: 13, fontWeight: 700, color: '#1E293B' }}>{t.ticket_code || t.ticket_id || `TKT-${t.id}`}</td>
