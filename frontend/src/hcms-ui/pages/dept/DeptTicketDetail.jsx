...
import { ChevronLeft, Clock, Download, Paperclip, Send, CheckCircle, XCircle, FileSearch, FileText, RotateCcw, MessageSquare, RefreshCw, UserPlus, ArrowUp, AlertCircle, AlertTriangle, X, Lock } from 'lucide-react';
...
const ACTIONS = [
  ...
  { key: 'in_progress', label: 'Start Investigation', Icon: FileSearch },
  { key: 'escalate', label: 'Escalate to Next Level', Icon: ArrowUp },
];

// Statuses where the DH can still take actions on the ticket
const ACTIONABLE_STATUSES = ['new', 'in_progress', 'escalated', 'waiting'];
...
function formatTimelineDesc(item) {
  ...
  // Default fallback - try to show something useful
...
export default function DeptTicketDetail() {
  ...
  const backPath = location.pathname.replace(/\/\d+$/, '') || '/hcms/dept-escalations';
  const backLabel = backPath.includes('assigned') ? 'My tickets' : 'Escalations';
  ...
  const [confirmModal, setConfirmModal] = useState(null); // { action, title, message, placeholder, requireReason }
    ...
    const fetchTicket = async () => {
      try {
        ...
        const ticket = caseData ? {
          ...caseData,
          ticket_id: caseData.ticket_code || caseData.ticket_id,
        ...
        setCaseAttachments(ticket?.attachments || []);
      } catch (err) {
  ...
  const refreshTicket = async (token) => {
  ...
  const performAction = async (actionKey, reason = '') => {
    try {
      ...
      } else if (actionKey === 'resolve' || actionKey === 'in_progress') {
        const newStatus = actionKey === 'resolve' ? 'resolved' : 'in_progress';
        const res = await fetch(`/api/v2/cases/${id}`, {
          method: 'PUT',
          headers,
  ...
  const handleConfirmAction = () => {
    ...
    if (confirmModal.requireReason && !reason) {
      return;
    }
  ...
  if (!ticket) return (
    <div style={{ fontFamily: f, padding: 40, textAlign: 'center', color: '#64748B' }}>
      <button onClick={() => navigate(backPath)} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 20, background: 'transparent', border: 'none', color: '#64748B', fontSize: 13, cursor: 'pointer', padding: 0 }}>
        <ChevronLeft size={16} />Back to {backLabel.toLowerCase()}
      </button>
      Ticket not found.
    </div>
  );
  ...
  // DH can only act on tickets that are escalated to them (pending_approval or under_investigation)
  const deptStatus = ticket.dept_review_status || ticket.status;
  const canAct = ACTIONABLE_STATUSES.includes(deptStatus) && canPerformActions(ticket);

  return (
    <div style={{ fontFamily: f }}>
      {/* Breadcrumb */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, fontSize: 13, color: '#64748B' }}>
        <button onClick={() => navigate(backPath)} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: '#64748B', fontSize: 13, cursor: 'pointer', padding: 0 }}>
      ...
      {/* View-only banner when ticket is not actionable */}
      {!canAct && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderRadius: 10, background: '#F1F5F9', border: '1px solid #E2E8F0', marginBottom: 20, fontSize: 13, color: '#64748B' }}>
          ...
          This ticket is view-only. Actions, conversation, and attachments are disabled because the ticket is not currently escalated to you.
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={card}>
            ...
            <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', rowGap: 20, columnGap: 24 }}>
              {[
                ...
                { label: 'ASSIGNED HR EXECUTIVE', value: ticket.assigned_hr_name || '—' },
                  ...
                  <div key={m.id} style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 12, color: '#94A3B8' }}>· {m.time || m.created_at ? new Date(m.created_at).toLocaleString() : '—'}</span>
                      </div>
              ...
              {canComment(ticket) ? (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <input type="file" multiple onChange={handleCommentAttachmentSelect} style={{ display: 'none' }} id="dept-comment-attachment-input" />
                      <label htmlFor="dept-comment-attachment-input" style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'transparent', border: 'none', color: '#64748B', fontSize: 13, fontWeight: 500, cursor: 'pointer', padding: 0 }}>
                ...
                <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '8px 0' }}>You do not have permission to add comments.</div>
              )}
            </div>
          </div>
          ...
          <div style={card}>
            <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #E2E8F0' }}>
              <div>
                <h2 style={{ fontSize: 14, fontWeight: 700, color: '#1E293B', margin: '0 0 2px' }}>Investigation Notes</h2>
                <p style={{ fontSize: 12, color: '#94A3B8', margin: 0 }}>Visible only to Department Head, HR, and System Admin</p>
              </div>
            </div>
            <div style={{ padding: '14px 20px' }}>
                ...
                internalNotes.map(n => (
                  <div key={n.id} style={{ padding: '12px 16px', background: '#F8FAFC', borderRadius: 10, border: '1px solid #F1F5F9', marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: '#94A3B8' }}>{n.time || n.created_at ? new Date(n.created_at).toLocaleString() : '—'}</span>
                    </div>
              ...
              {canComment(ticket) ? (
                <>
                  <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Add findings, observations, or remarks..." rows={3}
                ...
                <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '8px 0' }}>You do not have permission to add notes.</div>
              )}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={card}>
            ...
            <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {canAct ? (
                ACTIONS.filter(a => {
                  if (a.key === 'resolve') return canResolve(ticket);
                  if (a.key === 'escalate') return canPerformActions(ticket) && !ticket.pendingEscalationConsent;
                ...
                <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: '12px 0' }}>No actions available for this ticket status.</div>
              )}
            </div>
          </div>
              ...
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: '#F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {item.isPriority ? <AlertTriangle size={14} color="#64748B" /> : item.isStatus ? <CheckCircle size={14} color="#64748B" /> : item.slaColor ? <Clock size={14} color="#64748B" /> : <FileText size={14} color="#64748B" />}
                  </div>
                  <div>
      ...
      {confirmModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 16, width: 440, maxWidth: '90vw', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {confirmModal.requireReason && (
                <textarea
                  ...
                  placeholder={confirmModal.placeholder || 'Enter reason...'}
                  rows={3}
            ...
            <div style={{ padding: '0 24px 20px', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={() => setConfirmModal(null)}
                style={{ height: 36, padding: '0 16px', borderRadius: 8, background: '#FFFFFF', border: '1px solid #E2E8F0', color: '#475569', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleConfirmAction}
                disabled={confirmModal.requireReason && !(confirmModal.reasonInput || '').trim()}
                style={{
                  height: 36, padding: '0 16px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                  background: '#1E293B',
                  color: '#FFFFFF',
                  opacity: (confirmModal.requireReason && !(confirmModal.reasonInput || '').trim()) ? 0.5 : 1
