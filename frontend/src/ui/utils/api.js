...
export const isStaffRoute = () => {
  ...
  // FAQ module archived - removed faq-admin from staff session validation
  return /^\/(agentdashboard|agent-dashboard|agent\/|manager|manager-dashboard|ceo|tickets|ticket\/|products|businessdashboard|business-products|business-tickets)/.test(p) || p === '/tickets-table';
};
...
export const getApiBaseUrl = () => {
  ...
  // In production, use same-origin API through Nginx reverse proxy.
  // Example: https://itsm.vardaands.com/api/...
  return '';
};
 ...
 *
 * Supports both:
 *   buildApiUrl('/api/tickets') -> /api/tickets
 *   buildApiUrl('/tickets')     -> /api/tickets
 */
export const buildApiUrl = (endpoint = '') => {
  const baseUrl = getApiBaseUrl();
  const cleanEndpoint = endpoint.startsWith('/') ? endpoint : `/${endpoint}`;

  // If no base URL, return same-origin endpoint as-is.
  if (!baseUrl) {
  ...
  // Case 1: base is /api and endpoint is already /api/...
  if (cleanBase.endsWith('/api') && cleanEndpoint.startsWith('/api/')) {
    return `${cleanBase}${cleanEndpoint.slice(4)}`;
  }

  // Case 2: base is /api and endpoint is /tickets
  if (cleanBase.endsWith('/api') && !cleanEndpoint.startsWith('/api/')) {
    return `${cleanBase}${cleanEndpoint}`;
  }

  // Case 3: base is domain and endpoint is already /api/...
  if (!cleanBase.endsWith('/api') && cleanEndpoint.startsWith('/api/')) {
    return `${cleanBase}${cleanEndpoint}`;
  }

  // Case 4: base is domain and endpoint is /tickets
